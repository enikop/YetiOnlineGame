...
export class BlueYetiComponent implements OnInit, OnDestroy {
  ...
  processFeedback(feedbackData: any) {
    ...
    if (feedbackData.valid) {
      const player = this.players.filter(p => p.playerId == this.turnId)[0];
      this.previousPairs.push({ less: feedbackData.less, greater: feedbackData.greater, convergent: feedbackData.convergent, userName: player.userName });
  ...
  nextPlayerTurn(playersData: any) {
    ...
    for( let player of this.players){
      player.cardNumber = playersData.players.filter( (p:any) => p.playerId == player.playerId)[0].cardNumber;
    }
